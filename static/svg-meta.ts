...
// svg-meta.ts — XML traversal utilities and avatar metadata extraction.
...
import {
  type LocalizedText,
  type NonEmptyString, type IsoDateOnly, type IsoDateTimeWithOffset,
  type XmpDocumentId, type XmpInstanceId, type UuidUrn, type AssetIdentifier,
  type AvatarMeta,
} from "./avatar-meta-types.ts";
...
export function getByLocalPreferAlt(el: Undefinable<XmlElement>, localName: string): Undefinable<XmlElement> {
  ...
  return orElseForUndefinable(all.find((c) => isNotUndefined(getByLocal(c, "Alt"))), () => all[0]);
}
...
function extractBagItems(el: Undefinable<XmlElement>): readonly string[] {
  const bag = orElseForUndefinable(getByLocal(el, "Bag"), () => getByLocal(el, "Seq"));
